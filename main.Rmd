...
\noindent Once we have chosen $wpd$ as a relevant feature for characterizing the time series across one cyclic granularity, we have to decide how we combine differences between the multiple features (corresponding to multiple granularities) into a single number. We choose the euclidean distance between them with the granularities being the variables and $wpd$ being the value under each variable. With this approach, we should expect the observations with similar $wpd$ values to be clustered together. Thus this approach is useful to group customers that have similar significance of patterns across different granularities.
Similar significance do not imply similar pattern, and this how this approach differs from the JS-based distances which picks up differences in patters across categories.


<!-- Once we have chosen relevant features to characterize the probability distributions, we have to decide how to combine them into a single number, which serves as a distance between two observations. -->

<!-- which are described below. -->

<!-- Consequently, the distance measure might also change depending on the similarity measure chosen. -->


<!-- The (dis) similarity measure in this paper focuses on looking at the (dis) similarity between underlying distributions that may have resulted in different patterns across different cyclic temporal granularities and considers two methodology for finding distances between observations, which are explained in the next segment. -->
