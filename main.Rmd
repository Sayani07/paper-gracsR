...
The foundation of our method is unsupervised clustering algorithms based exclusively on time-series features. First, we study the underlying distributions that may have resulted in different patterns across temporal granularities in order to identify a mechanism to classify them based on the similarity of those distributions. Depending on the goal of clustering, the distance metric for defining similarity would be different. These distance metrics could be fed into a clustering algorithm to break large data sets into subgroups that can then be analyzed separately. These clusters may be commonly associated with real-world data segmentation. However, since the data is unlabeled a priori, more information is required to corroborate this.

This section presents the work flow of the methodology:

- _Find significant harmonies or cyclic granularities_:

For each key variable, the raw smart meter data is a sequence that is indexed by time and comprises values of several variables of interest (measurement variables) at each time point. This sequence, though, could be depicted in a variety of ways. A shuffling of the raw sequence could reflect the distribution of hourly consumption over a single day, while another could indicate consumption over a week or a year. These temporal deconstructions of a time period into units such as hour-of-day, work-day/weekend, called cyclic temporal granularities, are useful for exploring repetitive patterns in time series data that get lost in the linear representation of time. However, it is advantageous to consider those cyclic granularities across which there is a significant repetitive pattern for the majority of customers or noteworthy in an electricity-behavior context, because when the customers are grouped, we can expect to observe some interesting patterns across the categories of the cyclic granularities considered.
...
If we consider the sequence of measured variables for each category of any cyclic granularity to be from a single data generating process, we want to cluster distributions of the form $f_{i,j,k}(v)$, where $j$ and $k$ denote two granularities. For example, j=hour of day and k=day of week. We still want the households in clusters, but each household consists of a collection of probability distributions f_{i,h,d}(x) where $h=1,...,24$ and $d=1,...,7$. In this example (hour of day/day of week), we would have $24*7=168$ distributions per household if we consider interaction of the households or $24+7=31$ distributions if individual effect of granularities need to be considered. So we need a distance between collections of these univariate probability distributions. There could be several ways of considering such distances. We consider the following two approoaches:
