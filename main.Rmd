...
```{r opt-cluster-tsne-jsd, fig.cap = "Clustering summaries: t-SNE computed on the $24$ selected customers (a), and separation index ($sindex$) for 2-10 clusters using JS (b) and WPD (c). Various choices in number of clusters would be recommended. Four clusters are visible in t-SNE, although it might hide a fifth cluster because dimension reduction to 2D may be insufficient to see the difference. JS suggests 3 and wpd suggests 3 or 6."}
...
```{r groups-4and5, fig.cap = "Summary plots for four and five clusters. The distribution of electricity demand combined for all members over $hod$, $moy$, and $wnwd$ is shown. Groups A-1 (customers XXX), A-2 (customers XXX), and A-3 (customers XXX) profiles correspond to Groups B-1, B-2, and B-3, respectively. Cluster A-4 splits into B-4 (customers XXX) and B-5 (customers XXX)to produce the five clusters, whci h better resolves the $moy$ distribution. B-4 is characterized by higher energy consumption in the first few months of the year and more variation in weekend usage. B-5 (id:1-3) is distinguished by higher consumption in the middle of the year (winter months) and similar weekday-weekend inter-quartile range. When $k = 4$ is used, these two groups merge to form A-4, which has a moy profile of higher usage in both the beginning and middle of the year, which is not representative of the individuals in the group.", fig.height=5}
