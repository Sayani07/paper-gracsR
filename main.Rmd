...
For each key variable, the raw smart meter data is indexed by time and comprises values of several variables of interest (measurement variables) at each time point. This sequence, though, could be depicted in a variety of ways. One sequence could reflect the distribution of hourly consumption over a single day, while another could indicate consumption over a week or a year. The sequence of measured variables throughout each category of any cyclic granularity could be regarded as a single data generating process. It is beneficial to evaluate those cyclic granularities that are significant for the majority of customers or noteworthy in an electricity-behavior context, so that when the customers are grouped, we can expect to observe some interesting patterns across the categories of the cyclic granularities considered.
