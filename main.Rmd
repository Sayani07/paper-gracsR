...
Huge data sets presents more complications for clustering. Clustering algorithms work well when there well-separated clusters, with no nuisance variables or nuisance observations. When converting a series to granularities, many variables (each level of a granularity) are generated, possibly creating a slew of nuisance variables. Some customers may have a mix of energy use patterns, which could be considered nuisance observations, between major clusters. For this reason, we have chosen to select a small group of customers with relatively distinct and different patterns in order to illustrate the clustering, simply. This is often a good approach to tackling a big analysis task, so start with a simpler task. The approach, however, is applicable using on all customers, but it would be more complicated to make final choice of the number of clusters and explain the results.
