...
The median is represented by a line and the shaded region represents the area between $25^{th}$ and $75^{th}$ percentile.

```{r moy-combined, fig.cap = "The energy demand distribution for the prototype designs (left) and clustering (right) are shown. The customer grouping by design and the clustering approach largely match. To compare one-to-one, the customers are organised in the same order on both sides. Except for some customers, design 1, 2, 3, 4 and 5 match to group 4, 5, 1, 3, 2. Except in group 3, where consumers are categorised by greater usage in the middle of the year, which corresponds to winter in Australia. Since the designs are selected only based on 50th percentile, the clustering approach outperforms the design in a few circumstances."}
