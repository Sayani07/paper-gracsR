...
write_rds(data_validation, here(paste0("js-nqt/3gran_change_5D/data_validation_", scen, ".rds")))
...
# 
# (dist_mat_g1/2) %>% broom::tidy() %>% pull(distance) %>% sd
# (dist_mat_g2/3) %>% broom::tidy() %>% pull(distance) %>% sd
# (dist_mat_g3/5) %>% broom::tidy() %>% pull(distance) %>% sd
# ((dist_mat_g1/2) + (dist_mat_g2/3) +  (dist_mat_g3/5))%>% broom::tidy() %>% pull(distance) %>% sd
# 
# 
# total_variation_g1 <- (dist_mat_g1/2) %>% broom::tidy()%>% 
#   left_join(groups, by = c("item1" = "id")) %>% 
#   left_join(groups, by = c("item2" = "id")) %>% 
#   dplyr::filter(group.x==group.y) %>% 
#   group_by(group.x, group.y) %>% 
#   summarise(total = sum(distance)) %>% mutate(gran = "g1") 
# 
# total_variation_g2 <- (dist_mat_g2/3) %>% broom::tidy()%>% 
#   left_join(groups, by = c("item1" = "id")) %>% 
#   left_join(groups, by = c("item2" = "id")) %>% 
#   dplyr::filter(group.x==group.y) %>% 
#   group_by(group.x, group.y) %>% 
#   summarise(total = sum(distance)) %>% mutate(gran = "g2") 
# 
# total_variation_g3 <- (dist_mat_g3/5) %>% broom::tidy()%>% 
#   left_join(groups, by = c("item1" = "id")) %>% 
#   left_join(groups, by = c("item2" = "id")) %>% 
#   dplyr::filter(group.x==group.y) %>% 
#   group_by(group.x, group.y) %>% 
#   summarise(total = sum(distance)) %>% mutate(gran = "g3") 
# 
# bind_rows(total_variation_g1, total_variation_g2, total_variation_g3) %>%
#   group_by(gran) %>% 
#   mutate(sum = sum(total)) %>% 
#   mutate(percent_contr =  total*100/sum) %>% 
#   pivot_wider(-c(total, group.x), names_from = group.y, values_from = percent_contr) %>% select(-sum)
