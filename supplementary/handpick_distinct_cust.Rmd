...
# write_rds(elec_harmny, paste("../data/elec_harmony-100-nogap-new.rds"))
...
# data_pick %>% 
#   create_gran("day_month") %>% 
#   ungroup() %>% left_join(data_pcp_ge2 %>% select(customer_id, group)) %>% 
#   #filter(group==3) %>% 
#   #filter(customer_id %in% c(8148781, 10703878)) %>% 
#   ggplot(aes(x = as.factor(day_month), y = general_supply_kwh)) +
#   geom_boxplot(size = 0.5) +
#   facet_wrap(group~customer_id,
#              labeller = "label_value",
#              scales = "free_y") + 
#   theme_minimal()+ theme(strip.text.y = element_text(size = 5)) + scale_y_log10()
```
