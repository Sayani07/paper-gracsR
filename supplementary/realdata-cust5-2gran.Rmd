---
title: "Clustering real data with five customers and choosing optimal number of clusters for hours of day and day of week"
output:
  bookdown::pdf_book:
...
1. _Compute quantiles of distributions across each combination of hour of day and day of week _
2. _Compute JS distance between households for each combination of categories_
3. _Total distance between households computed as sum of JS distances for all combinations_
...
```{r raw-all, fig.cap="The raw time plots for demand shown for the entire observation period between 2012-2014 for 5 households (facets). The data is squeezed stopping us from seeing any behavioral patterns."}
library(lubridate)
...
The distribution of demand across different hours of the day observed for these five households through heatmaps (Figure \ref{fig:hod-deciles}) and quantile plots (Figure \ref{fig:hod-heatmap}). The following characterization is done to validate results of the clustering approach later on.

```{r}
sm_hod_dow <- sm %>% gravitas::create_gran("hour_day") %>% 
...
sm_hod_list <- sm_hod_dow %>% 
  as_tibble() %>% 
  arrange(customer_id, day_week, hour_day) %>% 
  mutate(category = paste(hour_day, day_week, sep = "-")) %>% 
   select(customer_id, category, general_supply_kwh) %>% 
...
ref_cat_split <- map(1:nrow(categories_ref), function(x){
  value = str_split(categories_ref$value[x], pattern = "-") %>% unlist() 
  hod = value[1]
  dow = value[2]
  tibble(hod = hod, dow = dow)
}) %>% bind_rows(.id = "categories_serial_id")
...
data_heatmap <- sm_hod_quantiles %>% 
  ...
  left_join(ref_cat_split) %>% 
  mutate(hod = as.integer(hod))

data_heatmap$dow <- factor(data_heatmap$dow, 
                           levels = c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"))

data_heatmap%>% 
ggplot(aes(x= customer_serial_id, 
           y = as.factor(quantile_prob))) +
...
```{r hod-deciles-new, fig.cap = "The deciles plots for 5 households are shown for entire observation period across hours of the day. The y-scales are made free to allow us to see daily patterns for each households. The deciles for all households tend to show an increase in the morning and evening hours, although the hours differ. Except for the 90th decile, the deciles for the 4th household look pretty flat."}
sm %>% filter(customer_id %in% c("10006704")) %>% 
           gravitas::prob_plot("day_week", "hour_day", plot_type = "quantile") +
  theme_bw()


sm %>% filter(customer_id %in% c("10017936")) %>% 
           gravitas::prob_plot("day_week", "hour_day", plot_type = "quantile") +
  theme_bw()

sm %>% filter(customer_id %in% c("10018064")) %>% 
           gravitas::prob_plot("day_week", "hour_day", plot_type = "quantile") +
  theme_bw()
```
...
all_data_clust = map_dfr(seq_len(nrow(ncust)), function(x){
  map(seq_len(length(sample_seed)), function(seed){
     ...
     mutate(customer_id = paste(customer_id, seed, sep = "-"),
             general_supply_kwh = general_supply_kwh + rnorm(1, 0, 0.03))
     data1
...
```{r raw-iter, fig.cap = "Five iterations for each customer are considered by adding random noise $(N(0, \\sigma^2)$, where $\\sigma^2$ is very small relative to the the variance of the data. The raw plots for all simulated dataset is shown to make sure they look similar to the structure."}
all_data_clust %>% 
  ggplot(aes(x = reading_datetime, y = general_supply_kwh)) +
  geom_line() +
...
```{r}
sm_hod_dow <- gravitas::create_gran(all_data_clust, "hour_day") %>% 
...
sm_dist_data <- sm_hod_quantiles %>% 
  ...
  nest() %>% 
  pivot_wider(names_from = categories_serial_id, values_from = data) %>% ungroup()
```
  ...
  map(seq_len(nrow_data), function(y){ # 2nd data
    map(seq_len(ncol_data), function(z){ # number of combinations nx*nfacet
      JS(
        ...
        sm_dist_data[-1] %>% extract(x, z) %>% unnest() %>% pull(quantile_val),
        sm_dist_data[-1] %>% extract(y, z) %>% unnest() %>% pull(quantile_val)
      ) %>% as_tibble()
