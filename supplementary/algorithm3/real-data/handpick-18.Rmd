...
data_pick_scaled <- data_pick %>%
    ...
    dplyr::mutate(kwh_scaled = stats::qqnorm(general_supply_kwh, plot.it=FALSE)$x) %>%
    ungroup()
    
quantile_prob_val = c(0.25, 0.5, 0.75)
  
#sm_hod_scaled <- gravitas::create_gran(data_pick_scaled, "hour_day")

sm_hod_all <- data_pick %>% 
bind_cols(data_pick_scaled %>% as_tibble() %>% select(kwh_scaled)) %>% 
  rename("kwh" = "general_supply_kwh") %>% 
gravitas::create_gran("hour_day") %>% 
  pivot_longer(c(5, 11), names_to = "kwh_type", values_to = "kwh_value")


sm_hod_all %>%
filter(design ==1) %>%
ggplot(aes(x = kwh_value, y = hour_day, fill = kwh_type)) +
ggridges::geom_density_ridges(alpha = 0.5) +
facet_wrap(~customer_id, scales = "free_x") +
  stat_summary(
    fun = median,
    geom = 'line',
    aes(group = 1), size = 0.5) +
    coord_flip() +
...
sm_hod_all %>%
filter(customer_id %in% c(8181071)) %>%
ggplot(aes(x = kwh_value, y = hour_day), fill = "blue") +
ggridges::geom_density_ridges(alpha = 0.5) +
facet_grid(customer_id~kwh_type, scales = "free_x") +
  stat_summary(
    fun = median,
    geom = 'line',
    aes(group = 1), size = 0.5, color = "blue") +
    #coord_flip() +
  theme(legend.position = "bottom")

sm_hod_all %>%
filter(customer_id %in% c(11013154)) %>%
ggplot(aes(x = kwh_value, y = hour_day), fill = "blue") +
ggridges::geom_density_ridges(alpha = 0.5) +
facet_grid(customer_id~kwh_type, scales = "free_x") +
  stat_summary(
...
# sm_hod_all %>%
# filter(customer_id %in% c(8955520)) %>%
# ggplot(aes(x = kwh_value, y = hour_day), fill = "blue") +
# ggridges::geom_density_ridges(alpha = 0.5) +
# facet_grid(customer_id~kwh_type, scales = "free_x") +
#   ggridges::stat_density_ridges(quantile_lines = TRUE, quantiles = 0.9) + stat_summary(
#     fun = quantile_q2,
#     geom = 'line',
#     aes(group = 1), size = 0.5, color = "blue") +
#     #coord_flip() +
...
sm_hod_all %>%
filter(customer_id %in% c(8198401)) %>%
ggplot(aes(x = kwh_value, y = hour_day), fill = "#999999") +
ggridges::geom_density_ridges(alpha = 0.7) +
facet_wrap(~kwh_type, scales = "free_y", ncol = 1) +
  stat_summary(
    fun = quantile_q2,
    geom = 'line',
...
# two peaks
sm_hod_all %>%
filter(customer_id %in% c(10469265)) %>%
ggplot(aes(x = kwh_value, y = hour_day), fill = "#999999") +
ggridges::geom_density_ridges(alpha = 0.7) +
facet_wrap(~kwh_type, scales = "free_y", ncol = 1) +
  stat_summary(
...
sm_hod_all %>%
filter(customer_id %in% c(10046512)) %>%
ggplot(aes(x = kwh_value, y = hour_day), fill = "#999999") +
ggridges::geom_density_ridges(alpha = 0.7) +
facet_wrap(~kwh_type, scales = "free_y", ncol = 1) +
  stat_summary(
...
#     fun = quantile_q1,
#     geom = 'line',
#     aes(group = 1), size = 0.5, color = "#D55E00") +
#   theme(legend.position = "bottom") +
#    stat_summary(
#     fun = quantile_q3,
#     geom = 'line',
...
p1 <- sm_hod %>% 
filter(design ==1) %>% 
ggplot(aes(x = general_supply_kwh, y = hour_day)) +
ggridges::geom_density_ridges(alpha = 0.05) +
facet_wrap(~customer_id, nrow = 6) + 
  stat_summary(
    fun = median,
    geom = 'line',
    aes(group = 1), size = 0.5) +
...
p2 <- gravitas::create_gran(data_pick_scaled, "hour_day") %>% 
filter(design ==1) %>% 
ggplot(aes(x = kwh_scaled, y = hour_day)) +
ggridges::geom_density_ridges(alpha = 0.05, fill = "grey") +
facet_wrap(~customer_id, ncol = 2, scales = "free_x") + 
  stat_summary(
    fun = median,
    geom = 'line',
    aes(group = 1), size = 0.5) +
