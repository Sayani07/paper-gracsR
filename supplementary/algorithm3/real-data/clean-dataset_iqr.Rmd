...
library(gracsr)
# v2 <- suppressWarnings(
#   scaled_dist_gran_iqr(data, "hour_day",
...
# 
# write_rds(v2, "../../../data/scaled_dist_nogap_2013_100_iqr.rds")

v2 <- read_rds("../../../data/scaled_dist_nogap_2013_100_iqr.rds")
...
data_pick_clust_clean <- read_rds("../../../data/data_pick_cust_clean.rds")
...
data_pick <- data %>% 
  left_join(data_pick_cust %>% filter(!(customer_id %in% c(8283102, 10208156)))) %>% 
filter(customer_id %in% data_pick_cust$customer_id) %>% select(customer_id,reading_datetime, general_supply_kwh, design) 
```
...
sm <- robust_scale_data(data_pick, "hour_day")
v3 <- suppressWarnings(plot_data(sm, "hour_day", response = "scaled_response", nfacet = 6))
```
