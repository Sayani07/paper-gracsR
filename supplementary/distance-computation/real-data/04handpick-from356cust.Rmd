...
data_heatmap <- sm_quantiles_ref %>% 
 left_join(elec_pick %>% distinct(customer_id)) %>% 
...
subplot(mds_total, mds_wpd, quantile_plot, nrows = 2, widths = c(.6, .4), titleX = TRUE) %>% layout(showlegend = FALSE) %>% highlight(on = "plotly_selected", dynamic = TRUE, selectize = TRUE, selected = attrs_selected(0.5), opacityDim = .2, off = "plotly_deselect")
...
data_pick_one <- total_distance %>% filter(item1 %in% 
                            c(8486921)) %>% group_by(item1) %>% arrange(total) %>% head(5) %>% 
  mutate(item1 = as.integer(as.character(item1)),
         item2 = as.integer(as.character(item2)))
...
# unique(c(data_pick_one$item1,data_pick_one$item2)) %>% as_tibble(),    unique(c(data_pick_two$item1,data_pick_two$item2)) %>% as_tibble(), c(11013154, 11045304, 11048034,11139841,11156027,11006388) %>% as_tibble(), .id = "design")
# 
...
data_heatmap %>% 
  ...
  facet_wrap(design~customer_id, 
             ...
             ncol = 6) +
    theme(strip.text = element_text(size = 10, margin = margin(b = 0, t = 0))) + xlab("hour-of-day") + ylab("demand (in Kwh)") + theme_bw()
