...
library(gravitas)
library(tsibble)
elec <- read_rds("../paper-gracsr/data/elec_nogap_2013_600.rds") %>% select(customer_id, reading_datetime, general_supply_kwh)
...
elec_iqr0 <- elec %>% as_tibble() %>% group_by(customer_id) %>%
  summarise(q2 = stats::median(general_supply_kwh, na.rm = TRUE),
            ...
            quantile = stats::quantile(general_supply_kwh, na.rm = TRUE, prob = seq(0.1, 0.9, 0.1))) %>% filter(iqr==0| q2==0)


elec %>% as_tibble() %>%
  filter((customer_id == 8164613)) %>%
  summarise(q2 = stats::median(general_supply_kwh, na.rm = TRUE),
            iqr = stats::IQR(general_supply_kwh, na.rm = TRUE))
