---
title: "Clustering real data with 10 customers and choosing optimal number of clusters"
output:
  bookdown::pdf_book:
...
```{r raw-all, fig.cap="The raw time plots for demand shown for the entire observation period between 2012-2014 for ten households (facets). The data is squeezed stopping us from seeing any behavioral patterns."}
library(lubridate)
...
```{r hod-box, fig.cap="The raw plots for ten households shown for Sept 2013-Oct 2013. The data is zoomed in to emphasize weekly, hourly or any behavior/patterns that they might have.", eval = FALSE}
sm %>% 
...
all_data_clust = map_dfr(seq_len(nrow(ncust)), function(x){
  map(seq_len(length(sample_seed)), function(seed){
     ...
     mutate(customer_id = paste(customer_id, seed, sep = "-"),
             general_supply_kwh = general_supply_kwh + rnorm(1, 0, 0.001))
     data1
...
The following figures show dendograms when we are using optimal number of clusters (k) from k = fpc::nselectboot(). We observe that the $4^{th}$ household tend to get split as different clusters as increase the number of clusters.
