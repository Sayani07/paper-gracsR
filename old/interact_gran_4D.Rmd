...
```{r nobs50, echo = TRUE}
ntimes_val = 1000 # nobs per combination 
```
...
```{r}
data_clust <- data_q %>% tsibble::as_tsibble(key = unique_data, index = time)
  
library(gracsr)
data_clust %>% scale_gran(method="nqt", response = "sim_data") 



%>%
  dist_gran(gran1="id_facet", gran2 = "id_x") %>% 
  dist_gran(clust_gran(kopt=4)
