...
g1 <- t %%2
g2 <- t %%3
g3 <- rep_len(rep(c(0,1,2), each = 3), length.out = length(t))
...
data_design_4 <- generate_design(t = 599, mu = c(0, 2, 4, 6, 8, 10))
data_design_5 <- generate_design(t = 599, mu = c(0, 2, 0, 4, 2, 4, 4, 6))
```
...
groups <- cutree(hc, k=5) %>% as_tibble() %>% bind_cols(comp) %>% 
