...
dist_mat <- bind_data_iter_tsibble %>%
  ...
  dist_wpd(harmony_tbl, response = "ts", nperm=10)

groups = dist_mat%>% clust_gran(kopt = 5)


data_validation <- dist_mat %>% broom::tidy() %>% 
...
data <- bind_data_iter_tsibble %>% left_join(groups, by = c("customer_id" = "id"))

write_rds(data_validation, here(paste0("validation/wpd/3gran_change_5D/data_validation_", scen, ".rds")))

write_rds(data, here(paste0("validation/wpd/3gran_change_5D/data_", scen, ".rds")))
